<?php
    ...
    class Generic extends Aux {
        ...
        public function all($d){
                ...
                if($valor == "true"){
                    ...
                    foreach($rest_tb as $item){
                        ...
                        if(key_exists(1,$field) && trim($field[0]) == 'id'){
                            ...
                            $tb          =  $field[1];
                            $tables     .=  "$tb ,";
                            $join       .=  "join $tb on  $tb.id = $table_name.$campo ";
                ...
                if(!empty($_REQUEST['moreTable'])){
                    foreach($more as $item){
                        if(!empty($item)){
                            ...
                            $q1          =  trim($vetTb[0]);
                            $tb1         =  explode('.',trim($q1))[0];
                            $q2          =  trim($vetTb[1]);
                            $tb2         =  explode('.',trim($q1))[0];
                            $per         =  $leftJoin ? 'left' : '';
                            $tables     .=  "$tb1 ,";
                            $join       .=  " $per join $tb1 on  $q1 = $q2 ";
                            $condicoes  .=  " and $q1 = $q2 ";
                            ...
                            foreach($tbCampo as $e){
                                $cmp     =  $e['Field'];
                                $sep     =  $isGraph ? '__' : "_";
                                $fields .=  "$tb1.$cmp as ".strtoupper($tb1).$sep.$cmp.", ";
                            }
                        }
                    }
                }
            ...
            if(isset($_REQUEST['subQuary'])){
                ...
                if(!empty($unir)){
                    ...
                    foreach($vet_unir as $i){
                        ...
                        $vet_result  = explode(' ',$i);

                        if(count($vet_result)==3){

                            $tb     =  $vet_result[0];
                            $tb_id  =  $tb.".".$vet_result[1];
                            $tb_id2 =  $table_name.".".$vet_result[2];
                            $data   =  $db->getDesc($tb);
                            ...
                            foreach($data as $line){
                                $campo    =   $line['Field'];
                                $sep      =   $isGraph  ? '__' : "_";
                                $fields  .=   " (select $tb.$campo from $tb where $tb_id = $tb_id2 limit 1) as _".strtoupper($tb).$sep.$campo.", ";
                            }
                        }
                    }
            ...
            $isOrder    =  isset($_REQUEST['isOrder']) ? $_REQUEST['isOrder'] : false; 
            $db         =  new Database();
            ...
            $query = !empty($fields) && !empty($join) ? "select $all_start $fields from $table_name $join $where" : "select * from $table_name $where ";
            ...
            /* Order */
            $query = $query." order by $table_name.id desc";     
