class validarCs {
  ...
  fillSelect = (
    data = [{ select, label, value: "id", table, method: "all_cmb" }]
  ) => {
    data.forEach((item) => {
      let { select, label, value = "id", table, method = "all_cmb" } = item;
    ...
    for (var e in data) {
      ...
      if (input)
        if (input.getAttribute("type") !== "file") {
          ...
          vet.map((e) => {
            if (e.trim() !== "") this.setInputFiles(input, Server.fileLink, e);
          });
        }
    }
